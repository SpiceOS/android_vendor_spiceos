function __print_spiceos_functions_help() {
cat <<EOF
Additional SpiceOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- spiceosgerrit:   A Git wrapper that fetches/pushes patch from/to SpiceOS Gerrit Review.
- spiceosrebase:   Rebase a Gerrit change and push it again.
- spiceosremote:   Add git remote for SpiceOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for SpiceOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Spice model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch spiceos_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/spiceos-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.spiceos.device | grep -q "$SPICEOS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SPICEOS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function spiceosremote()
{
    ...
    git remote rm spiceos 2> /dev/null
    ...
    local SPICEOS_USER=$(git config --get review.review.spiceosos.org.username)
    if [ -z "$SPICEOS_USER" ]
    then
        git remote add spiceos ssh://review.spiceosos.org:29418/$PFX$PROJECT
    else
        git remote add spiceos ssh://$SPICEOS_USER@review.spiceosos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'spiceos' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/SpiceOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.spiceos.device | grep -q "$SPICEOS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        spiceosremote
        git push spiceos HEAD:refs/heads/'$1'
    fi
    '
}

function spiceosgerrit() {
    ...
    local user=`git config --get review.review.spiceos.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "spiceosgerrit" ]; then
...
function spiceosrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "SpiceOS Gerrit Rebase Usage: "
        echo "      spiceosrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.spiceos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.spiceos.device | grep -q "$SPICEOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/spiceos/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SPICEOS_FIXUP_COMMON_OUT ]; then
