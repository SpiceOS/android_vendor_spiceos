...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop ro.spiceo.device | grep -q "$SPICEOS_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.spiceo.device | grep -q "$SPICEOS_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.spiceo.device | grep -q "$SPICEOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
