...
func init() {
	android.RegisterModuleType("spiceos_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = spiceosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = spiceosExpandVariables(ctx, dep_file)
	...
	cmd := spiceosExpandVariables(ctx, String(g.properties.Cmd))
